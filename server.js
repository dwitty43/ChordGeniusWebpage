...
async function deliverFile(res, finalChart, originalName, originalKey, targetKey, format, bpm, columns = '1') {
    let fileBuffer;
    let contentType;
    let extension;

    if (format === 'pdf') {
        fileBuffer = await createPdfChart(finalChart, originalName, originalKey, targetKey, bpm, columns);
...
app.get('/api/convert', async (req, res) => {
    ...
    try {
        ...
        await deliverFile(res, finalChart, query, songKey, targetKey, format, bpm, columns);
...
app.post('/api/binder', async (req, res) => {
    const { setlist, format, bpm, columns } = req.body; 
    ...
    try {
        ...
        await deliverFile(res, combinedText, "Setlist_Binder", "Mixed", "Mixed", format, bpm, columns || '1');
    } catch (error) {
...
app.post('/api/import', upload.single('chartFile'), async (req, res) => {
    ...
    try {
        ...
        await deliverFile(res, finalChart, originalName, finalSongKey, targetKey, format, bpm, columns);
