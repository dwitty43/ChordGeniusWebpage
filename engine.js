...
function transposeChord(chordString, originalKey, targetKey) {
    ...
    const targetKeyIndex = isTargetNashville ? 0 : getNoteIndex(targetKey.trim().replace(/m|min|minor$/i, ''));
    ...
    for (let line of lines) {
        ...
        if (isChordLine(line) || isNashvilleLine(line)) {
            ...
            let newLine = line.replace(/(\S+)(\s*)/g, (match, chord, spaces) => {
                let newChord = transposeChord(chord, currentOriginalKey, currentTargetKey);
